...
| URL | `/clock-corrections/pending-approval` |
...
│ ├──────────┼──────────┼────────┼──────────────────┼──────────────┼──┤   │
│ │ 佐藤花子 │ 02/01(日)│ 退勤   │ 17:00 → 19:00   │ 会議が延長...│  │   │
│ │ 田中一郎 │ 02/03(火)│ 出勤   │ --:-- → 09:00   │ ICカード不...│  │   │
│ │ 高橋美咲 │ 02/05(木)│ 両方   │ 09:30/17:00     │ 打刻機の故...│⚠│   │
│ │          │          │        │  → 09:00/18:00  │              │  │   │
│ └──────────┴──────────┴────────┴──────────────────┴──────────────┴──┘   │
├───────────────────────────────────────────────────────────────────────────┤
│ < 1 2 3 >                                             20件/ページ ▼   │
...
```
PendingClockCorrectionListPage
├── Breadcrumb
│   └── BreadcrumbItem ("ホーム", "申請管理", "承認待ち打刻修正一覧")
├── SearchForm (上部固定)
│   ├── TextInput (申請者名)
...
| 条件 | 表示 | 備考 |
|------|------|------|
| afterProvisionalClose = true | ⚠ (amber-500) | ツールチップ: 「仮締め後の修正申請です。人事承認が必要です」 |
...
| アクション | トリガー | 処理 | 権限 |
|-----------|---------|------|------|
| 検索 | 検索ボタン | 検索条件で一覧再取得 | read (上長+人事) |
...
| アクション | 処理 | 備考 |
|-----------|------|------|
| 行クリック | 打刻修正申請詳細画面 (SCR-CC-002) へ遷移 | 詳細画面で承認・却下を実行 |
...
/** 承認待ち打刻修正申請アイテム */
interface PendingClockCorrectionItem {
...
interface SearchParams {
  employeeName: string;            // デフォルト: 空（部分一致検索）
...
/** 画面全体の状態 */
interface PendingClockCorrectionListState {
  /** 検索条件 */
  ...
  /** データ */
  data: {
    content: PendingClockCorrectionItem[];
    page: {
      number: number;
      size: number;
...
|------|--------------|---------|-----------|
| 承認待ち一覧取得 | `/api/v1/clock-corrections/pending-approval` | GET | employeeName, dateFrom, dateTo, page, size, sort |
...
```json
{
  "content": [
    {
      "clockCorrectionRequestId": "CCR-20260401-EMP002",
      "employeeId": "EMP-002",
      "employeeName": "佐藤花子",
      ...
      "requestedAt": "2026-02-02T09:15:00+09:00"
    }
  ],
  "page": {
    "number": 0,
...
| Tablet (768-1024px) | 修正理由カラム非表示（5カラム） |
| Mobile (<768px) | カード形式に切り替え（申請者+対象日+修正種別+修正前→修正後+仮締め後フラグ） |
...
| 仮締め後アイコン: ホバー | ツールチップ表示（「仮締め後の修正申請です。人事承認が必要です」） |
...
- [x] 全コマンドに対応する操作UIがある（行クリック → 詳細画面遷移 → 承認/却下アクション）
- [x] エラー状態の表示が定義されている（Toast通知 + 再試行ボタン、権限エラー対応）
- [x] 状態遷移がUI上で表現されている（承認待ち一覧は SUBMITTED のみ表示、行クリックで詳細画面へ、仮締め後⚠フラグ）
